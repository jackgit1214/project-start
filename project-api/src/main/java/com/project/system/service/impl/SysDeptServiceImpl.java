...
//        TransactionStatus transactionStatus= TransactionAspectSupport.currentTransactionStatus();
    ...
    public List<SysDept> getDepartmentsBySuperId(String superId,boolean includeSelf) {
        QueryModel queryModel = new QueryModel();
        return this.getDepartmentsBySuperId(queryModel,superId,includeSelf);
    }
    ...
    public List<SysDept> getDepartmentsBySuperId(QueryModel queryModel, String superId, boolean includeSelf) {
        ...
        //显示下级所有部门及本部门
        List criterion = queryModel.getOredCriteria();
        if (criterion.size()>0)
            queryModel.getOredCriteria().get(0).andEqualTo("superId", superId);
        else
            queryModel.createCriteria().andEqualTo("superId", superId);
        if (includeSelf)
